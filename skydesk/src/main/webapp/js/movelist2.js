;(function($){
	...
	var getrobj = function(boxr){
		var lis = $(boxr).find('li');
		var obj = {};
		if(lis.length>0){
			for(var i=0;i<lis.length;i++){
				var key = $(lis[i]).data('key');
	...
	var resetbox = function(box,obj){
		if(obj==undefined)
			obj = {};
		for(var i in obj){
			var li = '<li data-key="'+i+'" data-value="'+obj[i]+'">'+obj[i]+'</li>';
			$(box).append(li);
		}
	}
	...
	var setlistbox = function(options){
		...
		var hzsjlistbox = options.hzsjlistbox;
		...
		for(var i in hzlist){
			...
			var lihtml = '<div class="skylistbox_left"><input type="checkbox" /></div>'
				+'<div class="skylistbox_left">'+hzlist[i].hzname+'</div>';
			$li.html(lihtml);
			if(hzlist[i].hzbj==1)
				$(hzlistbox).append($li);
			else
				$(hzsjlistbox).append($li);
		}
	...
	$.fn.skyListBox = function(options,param){
		if(typeof options === "string")
			return $.fn.skyListBox.methods[options](this,param);
		return this.each(function(){
			if(options){
				...
				var $hzsjlistbox = $(opts.hzsjlistbox);
				...
				$this.delegate('input[type=checkbox]','change',function(){
					var $this = $(this);
				...
				$this.find('li').click(function(){
					var ck = $(this).find('li input[type="checkbox"]');
					if($(ck).prop('checked'))
						$(ck).removeProp('checked');
					else
						$(ck).prop('checked',true);
				});
				$this.find('input.selectAll').click(function(){
					var id = $(this).parent().data('listbox');
					var ishz = $(this).parent().data('ishz');
					if(!ishz){
						$(id).find('li input[type="checkbox"]').prop('checked',true);
						$(id).find('li').addClass('checked');
					}
					else{
						$(id).find('li:not(.no_hz) input[type="checkbox"]').prop('checked',true);
						$(id).find('li:not(.no_hz)').addClass('checked');
					}
				});
				$this.find('input.clearAll').click(function(){
					var id = $(this).parent().data('listbox');
					var ishz = $(this).parent().data('ishz');
					$(id).find('li input[type="checkbox"]').removeProp('checked');
					$(id).find('li').removeClass('checked');
				});
				$hzlistbox.dragsort({ dragSelector: "li", dragBetween: false, dragEnd: function(){},scrollContainer: opts.hzlistbox, placeHolderTemplate: "" });
				$hzsjlistbox.dragsort({ dragSelector: "li", dragBetween: false, dragEnd: function(){},scrollContainer: opts.$hzsjlistbox, placeHolderTemplate: "" });
	...
	$.fn.skyListBox.methods = {
			...
			gethzsjname: function(jq){
				var options = $.data(jq[0],'skylistbox');
				var lis = $(options.hzsjlistbox).find('li.checked');
				var hzstr = "";
				if(lis.length>0){
					for(var i=0;i<lis.length;i++){
						var item =  $(lis[i]).data('listitem');
