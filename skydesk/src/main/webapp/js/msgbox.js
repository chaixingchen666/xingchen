...
ZENG.string = {
	...
	format: function(str) {
		...
		return str.replace(ZENG.string.RegExps.format,
		function(m, n) {
...
ZENG.msgbox.show = function(msgHtml, type, timeout, opts) {
	...
	mBox.id = "q_Msgbox";
	mBox.style.display = "";
	if ($('.msg-modal').length == 0) $('.zeng_msgbox_layer_wrap').wrap('<div class="msg-modal"></div>');
	else $('.msg-modal').show();
...
function alertmsg(i, t) {
	...
	ZENG.msgbox.show(tip, i, t)
}
function alerthide() {
	ZENG.msgbox._hide()
}
function alerttext(text, time) {
	ZENG.msgbox._hide();
	hasmsg = true;
	var t = time == undefined || time == "" ? 3000 : time;
	ZENG.msgbox.show(text, 2, t)
}
function clickautohide(i) {
	...
	ZENG.msgbox.show(tip, i, 10000);
	setTimeout(hidemsgmodal, 10000)
}
