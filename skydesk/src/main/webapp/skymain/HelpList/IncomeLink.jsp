<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <!-- 	勾对 -->
<div id="currcheckd" title="应收款勾对" style="width: 800px; height: 400px;"  class="easyui-dialog" closed=true>
<div id="currchcek_toolbar" class="toolbar">
<div class="toolbar_box1">
<input type="hidden" id="currcheck_pnoteno">
<input type="hidden" id="currcheck_custid">
...
function setcurrcheckt(){
	...
	$('#currcheckt').datagrid({
		...
		columns: [
			[{
				...
				formatter: function(value,row,index){
					...
					if(gdcurr0!=0)
						return '<input type="button" class="m-btn" value="取消勾对" onclick="incomecurrcheck(' + index + ')">';
					return '<input type="button" class="m-btn" value="勾对" onclick="incomecurrcheck(' + index + ')">';
				}
			}]
		],
		...
		pageSize: 20
	}).datagrid("keyCtr", "incomecurrcheck()");
}
//打开勾对窗
...
function incomecurrlink(page){
	var custid = $('#currcheck_custid').val();
	var houseid = $('#currcheck_houseid').val();
	var pnoteno = $('#currcheck_pnoteno').val();
	if(custid.length==0&&houseid.length===0&&pnoteno.length==0){
		alerttext("无效收款单据！");
		return;
	}
	$.ajax({
...
//勾对金额
function incomecurrcheck(index){
	var $dg = $("#currcheckt");
	if(index>=0)
			...
			try {
				if (valideData(data)) {
					...
					$dg.datagrid("updateRow",{
						index: index,
						row: row
					}).datagrid('reloadFooter').datagrid("refreshRow",index);
