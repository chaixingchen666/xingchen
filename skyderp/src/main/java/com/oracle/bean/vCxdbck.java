...
public class vCxdbck {
	...
	public void setZtbj(Integer ztbj) {
		Ztbj = ztbj;
	}
	...
	public String GetTable2Json(QueryParam qp, int sizenum) {
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) {
				...
				else if (strKey.equals("TOHOUSEID"))
					tohouseid = Long.parseLong(strValue);
			...
			strSql1.append("\n FROM allotoutm a");
			strSql1.append("\n join allotouth b on a.accid=b.accid and a.noteno=b.noteno");
			...
			if (noteno.length() > 0)
				...
				if (tohouseid > 0) {
					strSql1.append(" and b.tohouseid=" + tohouseid);
				} else if (!Tohouseidlist.equals("")) {
					strSql1.append(" and " + Func.getCondition("b.tohouseid", Tohouseidlist));
				}
			}
	...
	public Table GetTable(QueryParam qp) {
		...
		strSql.append("\n left outer join warehouse h on a.houseid=h.houseid");
		strSql.append("\n left outer join warehouse i on a.tohouseid=i.houseid");
		...
		if (Ztbj == 1)  //只查在途
			strSql.append("\n and (length(a.noteno1)<=0 or a.noteno1 is null) ");
		...
		if (!Tohouseidlist.equals(""))
			strSql.append(" and " + Func.getCondition("a.tohouseid", Tohouseidlist));
