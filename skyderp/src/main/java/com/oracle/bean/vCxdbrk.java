...
/**
 * @author Administrator 调拨入库汇总
 */
public class vCxdbrk {
	...
	private String Fromhouseidlist = "";
	...
	public String GetTable2Json(QueryParam qp, int sizenum) {
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) {
				...
				else if (strKey.equals("FROMHOUSEID"))
					fromhouseid = Long.parseLong(strValue);
			...
			strSql1.append("\n FROM allotinm a");
			strSql1.append("\n join allotinh b on a.accid=b.accid and a.noteno=b.noteno");
			...
			if (noteno.length() > 0)
				...
				if (fromhouseid > 0) {
					strSql1.append(" and b.fromhouseid=" + fromhouseid);
				} else if (!Fromhouseidlist.equals("")) {
					strSql1.append(" and " + Func.getCondition("b.fromhouseid", Fromhouseidlist));
				}
				if (operant.length() > 0)
	...
	public Table GetTable(QueryParam qp) {
		...
		strSql.append("\n left outer join warehouse h on a.houseid=h.houseid");
		strSql.append("\n left outer join warehouse i on a.fromhouseid=i.houseid");
	...
	// if (Overbj < 2)
		...
		if (!Fromhouseidlist.equals(""))
			strSql.append(" and " + Func.getCondition("a.fromhouseid", Fromhouseidlist));
