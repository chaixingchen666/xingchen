...
//*************************************
public class Custcheckm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=custcheckm_id.nextval; ");
		strSql.append("   insert into custcheckm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update custcheckm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from custcheckm");
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n    v_notedate date;";
		qry += "\n begin ";
		qry += "\n   select notedate into v_notedate from custcheckh where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    v_amount:=" + Amount + "; ";
		// =======================
		qry += "\n    v_count:=0;";
		if (barcode.length() >= 4) {
		...
		qry += "\n     select count(*) into v_count from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        values (custcheckm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_bookamt,v_amount,v_amount-v_bookamt,v_price,v_curr,'');";
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid";
		qry += "\n        and rownum=1 order by id ;";
		qry += "\n        update custcheckm set checkamt=checkamt+v_amount,amount=amount+v_amount, curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + ";";
		qry += "\n     end if; ";
		// ===============================
		...
		qry += "\n            values (custcheckm_id.nextval," + Accid + ",'" + Noteno + "',v_row.wareid,v_row.colorid,v_row.sizeid,v_bookamt,v_checkamt,v_checkamt-v_bookamt,v_row.retailsale,v_curr,'',sysdate);";
		qry += "\n        end loop;";
		qry += "\n     end;";
		// ===============================

		// qry += " update custcheckh set totalcurr=(select nvl(sum(curr),0) from custcheckm where custcheckh.accid=custcheckm.accid and custcheckh.noteno=custcheckm.noteno) ";
		// qry += " ,totalamt=(select nvl(sum(amount),0) from custcheckm where custcheckh.accid=custcheckm.accid and custcheckh.noteno=custcheckm.noteno ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n     update custcheckh set totalcurr=v_totalcurr,totalamt=v_totalamt  where accid=" + Accid + " and noteno='" + Noteno + "'; ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		qry += "\n    v_custid  number(10);";
		qry += "\n begin ";
		qry += "\n    select notedate,custid into v_notedate,v_custid from custcheckh where accid=" + Accid + " and noteno='" + Noteno + "';";
		String sizelist = "^";
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				qry += "\n  begin ";
				qry += "\n    select id into v_id from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" //
						+ Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n    update custcheckm set checkamt=checkamt+" + Amount + " where accid=" + Accid + " and noteno='" + Noteno + "' and id=v_id;";
				qry += "\n  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n    v_id:=custcheckm_id.nextval;";
				qry += "\n    v_bookamt:=f_getcustsumskux(to_char(v_notedate,'yyyy-mm-dd hh24:mi:ss')," + Accid + "," + Wareid + "," + Colorid + "," + Sizeid + ",v_custid,'" + Calcdate + "');";
				...
				qry += "\n  end; ";
				qry += "\n  update custcheckm set amount=checkamt-bookamt,curr=(checkamt-bookamt)*price where accid=" + Accid + " and noteno='" + Noteno + "' and id=v_id;";
			}
		}
		// ===============================
		// 将商品，颜色未输入的尺码自动填为0，表示数量为0
		...
		qry += "\n     begin";
		qry += "\n        for v_row in cur_data loop";
		qry += "\n            v_bookamt:=f_getcustsumskux(to_char(v_notedate,'yyyy-mm-dd hh24:mi:ss')," + Accid + ",v_row.wareid,v_row.colorid,v_row.sizeid,v_custid,'" + Calcdate + "');";
		qry += "\n            v_curr:=(v_checkamt-v_bookamt)*v_row.retailsale;";
		qry += "\n            insert into custcheckm (id,accid,noteno,wareid,colorid,sizeid,bookamt,checkamt,amount,price,curr,remark0,lastdate)";
		qry += "\n            values (custcheckm_id.nextval," + Accid + ",'" + Noteno + "',v_row.wareid,v_row.colorid,v_row.sizeid,v_bookamt,v_checkamt,v_checkamt-v_bookamt,v_row.retailsale,v_curr,'',sysdate);";
		qry += "\n        end loop;";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n begin ";
		qry += "\n    select notedate,custid into v_notedate,v_custid from custcheckh where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    update custcheckm set checkamt=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "\n begin";
			qry += "\n   select id into v_id from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " ; ";
			qry += "\n   update custcheckm set checkamt=checkamt+" + Amount + ",amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
			qry += "\n EXCEPTION WHEN NO_DATA_FOUND THEN";
			qry += "\n   v_bookamt:=f_getcustsumskux(to_char(v_notedate,'yyyy-mm-dd hh24:mi:ss')," + Accid + "," + Wareid + "," + Colorid + "," + Sizeid + ",v_custid,'" + Calcdate + "' );";
			qry += "\n   v_curr:=round((" + Amount + "-v_bookamt)*" + Price + ",2);";
			qry += "\n   insert into  custcheckm (id,accid,noteno,wareid,colorid,sizeid,bookamt,checkamt,amount,price,curr,remark0)";
			qry += "\n   values (custcheckm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + ",v_bookamt," + Amount + "," + Amount + "-v_bookamt," + Price + ",v_curr,'');";
			qry += "\n end; ";
		}
		qry += "\n  delete custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";
		...
		qry += "\n  update custcheckh set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
	...
	public int Write(JSONObject jsonObject, long custid) {
		...
		qry += "\n  begin ";
		qry += "\n   select notedate into v_notedate from custcheckh where accid=" + Accid + " and noteno='" + Noteno + "';";
		// qry += "\n delete from custcheckm where accid=" + accid + " and noteno='" + noteno + "' and wareid=" + wareid + "; ";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "\n    v_bookamt:=f_getcustsumskux(to_char(v_notedate,'yyyy-mm-dd hh24:mi:ss')," + Accid + "," + Wareid + "," + Colorid + "," + Sizeid + "," + custid + ",'" + Calcdate + "');";

			qry += "\n    v_checkamt:=" + Amount + "; ";
			qry += "\n    begin ";
			qry += "\n      select id into v_id from custcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + ";";
			qry += "\n      update custcheckm set checkamt=v_checkamt,bookamt=v_bookamt,amount=v_checkamt-v_bookamt,curr=(v_checkamt-v_bookamt)*v_retailsale" //
		...
		qry += "\n            v_bookamt:=f_getcustsumskux(to_char(v_notedate,'yyyy-mm-dd hh24:mi:ss')," + Accid + ",v_row.wareid,v_row.colorid,v_row.sizeid," + custid + ",'" + Calcdate + "');";
		qry += "\n            insert into custcheckm (id,accid,noteno,wareid,colorid,sizeid,bookamt,checkamt,amount,price,curr,remark0,lastdate)";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from custcheckm a where accid=" + Accid + " and noteno='" + Noteno + "'";
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from custcheckm c ";
		...
		if (custid > 0) // 如果是采购退货，返回库存
		{
			...
			qry += "\n    from v_khkczy_data a where epid=" + Userid + " and wareid=" + Wareid + " and custid=" + custid;
			qry += "\n    and amount<>0 ";
			tb = DbHelperSQL.Query(qry).getTable(1);
			fh = "";
			rs = tb.getRowCount();
