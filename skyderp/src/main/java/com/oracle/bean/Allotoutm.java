...
//*************************************
//  @author:sunhong  @date:2017-03-08 20:55:44
//*************************************
public class Allotoutm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
		strSql.append("\n   select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   commit;");
		strSql.append("\n end;");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=allotoutm_id.nextval; ");
		strSql.append("   insert into allotoutm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update allotoutm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from allotoutm");
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n     begin";
		qry += "\n       select b.pricetype1 into v_pricetype from allotouth a join warehouse b on a.tohouseid=b.houseid where a.accid=" + Accid + " and a.noteno='" + Noteno + "';";
		qry += "\n     EXCEPTION WHEN NO_DATA_FOUND THEN";
		qry += "\n       v_pricetype:=0;";
		qry += "\n     end;";
		...
		qry += "\n     ,decode(b.tjtag,1,b.retailsale,case v_pricetype when 1 then b.sale1 when 2 then b.sale2 when 3 then b.sale3 when 4 then b.sale4 when 5 then b.sale5 else b.retailsale end) ";
		...
		qry += "\n     select count(*) into v_count from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        values (allotoutm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update allotoutm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += "\n update allotouth set totalcurr=(select nvl(sum(curr),0)
		// from allotoutm where allotouth.accid=allotoutm.accid and
		// allotouth.noteno=allotoutm.noteno) ";
		// qry += "\n ,totalamt=(select nvl(sum(amount),0) from allotoutm where
		// allotouth.accid=allotoutm.accid and allotouth.noteno=allotoutm.noteno
		// ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n     update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt  ";
		...
		qry += "\n        values (warebarrecord_id.nextval," + Accid + ",'" + barcode + "',7,'" + Noteno + "',v_amount,sysdate); ";
		qry += "\n     end if;";
		qry += "\n     commit; ";
		qry += "\n     <<exit>> ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				qry += "\n   begin ";
				qry += "\n     select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update allotoutm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into allotoutm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
				qry += "\n     values (allotoutm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int Load(long houseid, long tohouseid, int priceprec, String wareno) {
		if (Noteno.equals("")) {
		...
		if (tohouseid > 0) {
			...
			// System.out.println(qry + " " + pricetype);
		}
		if (Wareid != null && Wareid > 0) {
			qry = "select wareid,wareno,warename,retailsale,sale1,sale2,sale3,sale4,sale5,units,imagename0";
			qry += ",decode(tjtag,1,retailsale,case " + pricetype + " when 1 then sale1 when 2 then sale2 when 3 then sale3  when 4 then sale4  when 5 then sale5 else retailsale end) as price";
			qry += " from warecode where accid=" + Accid + " and statetag=1  and wareid=" + Wareid;
		} else if (wareno.length() > 0) {
			qry = "select wareid,wareno,warename,retailsale,sale1,sale2,sale3,sale4,sale5,units,imagename0";
			qry += ",decode(tjtag,1,retailsale,case " + pricetype + " when 1 then sale1 when 2 then sale2 when 3 then sale3  when 4 then sale4  when 5 then sale5 else retailsale end) as price";
			qry += " from warecode where accid=" + Accid + " and statetag=1 and rownum=1 and wareno='" + wareno + "'";
			// qry = "select
			// wareid,wareno,warename,entersale,retailsale,sale1,sale2,sale3,sale4,sale5,units,imagename0
			// from warecode where accid=" + Accid + " and wareno='" + wareno +
		...
		Price = Float.parseFloat(tb.getRow(0).get("PRICE").toString());
		// Discount = (float) 1;
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from allotoutm c ";
		...
		for (int i = 0; i < rs; i++) {
			...
			retcs += fh + "{\"COLORID\":\"" + tb.getRow(i).get("COLORID").toString() + "\"" //
					...
					// + ",\"PRICE0\":\"" + Price0 + "\""//
					// + ",\"DISCOUNT\":\"" +
					// tb.getRow(i).get("DISCOUNT").toString() + "\"" //
	...
	public int Write(JSONObject jsonObject) {
		...
		// noteno='" + noteno + "' and wareid="+wareid+"; ";
		qry += "\n      update allotoutm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		String sizelist = "^";
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "\n   begin ";
			qry += "\n     select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "\n     update allotoutm set amount=" + Amount + ",price=" + Price + ",curr=" + Curr + " where id=v_id and accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
			qry += "\n     insert into allotoutm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0,lastdate)";
			qry += "\n     values (allotoutm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + " ," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'',sysdate);";
			qry += "\n  end; ";
		}
		qry += "\n      delete from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
		qry += "\n      select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n      update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt  ";
		qry += "\n      where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n      commit; ";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		// qry += "\n ,case sum(a.amount) when 0 then 0 else
		// round(sum(a.amount*a.price0)/sum(a.amount)," + priceprec + ") end as
		// price0";
		// qry += "\n ,case sum(a.amount*a.price0) when 0 then 1 else
		// round(sum(a.curr)/sum(a.amount*a.price0),2) end as discount";
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from allotoutm a where accid=" + Accid + " and noteno='" + Noteno + "'";
	...
	// 复制单据
	public int doCopy(String fromnoteno, int fromnoteid, String tonoteno, int priceprec) {
		if (fromnoteno.length() <= 0 || tonoteno.length() <= 0) {
			errmess = "单据号参数无效！";
			return 0;
		}
		if (fromnoteno.equals(tonoteno) && fromnoteid == 6) {
		...
		qry += "\n    select nvl(b.pricetype1,0) into v_pricetype ";
		qry += "\n    from allotouth a join warehouse b on a.tohouseid=b.houseid where a.accid=" + Accid + " and a.noteno='" + tonoteno + "';";

		qry += "\n    declare cursor cur_data is ";
		qry += "\n      select a.wareid,a.colorid,a.sizeid,a.amount,a.remark0";
		qry += "\n      ,case v_pricetype when 1 then b.sale1 when 2 then b.sale2 when 3 then b.sale3 when 4 then sale4 when 5 then b.sale5 else b.retailsale end as price";
		// fromnoteid:0=采购，1=采购退货，2=店铺零售，3=批发，4=批发退货，5=商场零售，6=调出，7=调入
		...
		qry += "\n         begin ";
		qry += "\n           select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + tonoteno + "' and wareid=v_row.wareid and colorid=v_row.colorid and sizeid=v_row.sizeid and rownum=1; ";
		qry += "\n           update allotoutm set amount=amount+v_row.amount,curr=(amount+v_row.amount)*price where id=v_id and accid=" + Accid + " and noteno='" + tonoteno + "';";
		qry += "\n         EXCEPTION WHEN NO_DATA_FOUND THEN";
		qry += "\n           insert into allotoutm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
		qry += "\n           values (allotoutm_id.nextval," + Accid + ",'" + tonoteno + "',v_row.wareid,v_row.colorid,v_row.sizeid,v_row.amount,v_row.price,round(v_row.amount*v_row.price,2),v_row.remark0);";
		qry += "\n         end; ";
		qry += "\n         v_count:=v_count+1;";
		qry += "\n         if v_count>500 then ";
		qry += "\n            v_count:=1;";
		...
		qry += "\n      end loop;";
		qry += "\n    end;";
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotoutm where accid=" + Accid + " and noteno='" + tonoteno + "';";
		qry += "\n    update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + tonoteno + "';";
		qry += "\n    commit;";
		qry += "\n end;";
		if (DbHelperSQL.ExecuteSql(qry) < 0) {
			errmess = "操作失败！";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec, int fs) {
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "\n  begin ";
			qry += "\n    select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "\n    update allotoutm set amount=amount+" + Amount + ",curr=round((amount+" + Amount + ")*price,2) where id=v_id;";
		...
		qry += "\n  update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
		qry += "\n  where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n  commit; ";
		if (statetag == 1 && fs == 0 && houseid > 0) // 如果是退货，要判断负库存
		{
			qry += "\n  select notedate into v_notedate from allotouth where accid=" + Accid + " and noteno='" + Noteno + "';";
			// 要判断负库存
			qry += "\n begin";
		...
		if (statetag == 1) {
			qry += "\n   update allotouth set statetag=1,notedate=sysdate,lastdate=sysdate";
			qry += "\n   where accid=" + Accid + " and noteno='" + Noteno + "';";
		}
		qry += "\n   commit; ";
		qry += "\n   v_retcs:='1操作成功！' ; ";
	...
	public int LoadFromXLS(long tohouseid) {
		...
		if (Price != null) { // 如果传入了单价，以传入的价为准
			...
			if (tohouseid > 0) {
				qry += "\n   begin ";
				qry += "\n     select pricetype1 into v_pricetype from warehouse where accid=" + Accid + " and houseid=" + tohouseid + ";";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
				qry += "\n     v_pricetype:=0; ";
				qry += "\n   end; ";
			}
			qry += "\n   begin ";
			qry += "\n     select wareid,sizegroupno ";
			qry += "\n     ,case v_pricetype when 1 then sale1 when 2 then sale2 when 3 then sale3 when 4 then sale4 when 5 then sale5 else retailsale end";
			qry += "\n     into v_wareid,v_sizegroupno,v_price from warecode where accid= " + Accid + " and wareno='" + Wareno + "' and statetag=1 and rownum=1;";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
			qry += "\n     v_retcs:= '0货号:" + Wareno + " 未找到!' ; ";
			qry += "\n     goto exit; ";
			qry += "\n   end; ";
		...
		qry += "\n   end; ";
		qry += "\n    begin ";
		qry += "\n      select id into v_id from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update allotoutm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
		qry += "\n    EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
		qry += "\n      v_curr:=round(v_price*" + Amount + ",2);";
		qry += "\n      insert into allotoutm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
		qry += "\n      values (allotoutm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_price ,v_curr ,'');";
		qry += "\n    end; ";
		qry += "\n    select  nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotoutm where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    update allotouth set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    v_retcs:='1操作成功!' ; ";
