...
//*************************************
public class Firstcustm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
		strSql.append("\n   select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   update Firstcusth set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   commit;");
		strSql.append("\n end;");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=Firstcustm_id.nextval; ");
		strSql.append("   insert into Firstcustm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update Firstcustm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public Table GetTable(QueryParam qp, String strWhere, String fieldlist) {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select " + fieldlist);
		strSql.append(" FROM Firstcustm a");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from Firstcustm");
	...
	public int LoadFromXLS() {
		...
		qry += "\n   end; ";
		qry += "\n    begin ";
		qry += "\n      select id into v_id from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update Firstcustm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
		qry += "\n    EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
		qry += "\n      v_curr:=round(v_price*" + Amount + ",2);";
		qry += "\n      insert into Firstcustm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
		qry += "\n      values (Firstcustm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_price ,v_curr ,'');";
		qry += "\n    end; ";
		qry += "\n    select  nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    update Firstcusth set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    v_retcs:='1操作成功!' ; ";
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n     select count(*) into v_count from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        insert into Firstcustm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0) ";//
		qry += "\n        values (Firstcustm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update Firstcustm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += "\n update Firstcusth set totalcurr=(select nvl(sum(curr),0) from Firstcustm where Firstcusth.accid=Firstcustm.accid and Firstcusth.noteno=Firstcustm.noteno) ";
		// qry += "\n ,totalamt=(select nvl(sum(amount),0) from Firstcustm where Firstcusth.accid=Firstcustm.accid and Firstcusth.noteno=Firstcustm.noteno ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				Curr = Func.getRound(Amount * Price, 2);
				qry += "\n   begin ";
				qry += "\n     select id into v_id from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update Firstcustm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into Firstcustm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
				qry += "\n     values (Firstcustm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n    v_retcs  varchar2(100);";
		qry += "\n begin ";
		qry += "\n    update Firstcustm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "  begin ";
			qry += "    select id into v_id from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update Firstcustm set amount=amount+" + Amount + ",curr=round((amount+" + Amount + ")*price,2) where id=v_id;";
			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
			qry += "    insert into Firstcustm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
			qry += "    values (Firstcustm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
			qry += " end; ";
		}
		qry += "\n  delete Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";

		qry += "\n  select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "';";

		qry += "\n  update Firstcusth set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
		...
		qry += "\n     where accid=" + Accid + " and noteno='" + Noteno + "';";
		// if (statetag == 1) {
		// qry += "\n update Firstcusth set statetag=1,notedate=sysdate,lastdate=sysdate";
		// qry += "\n where accid=" + Accid + " and noteno='" + Noteno + "';";
		// }
		qry += "\n   commit; ";
		qry += "\n   v_retcs:='1操作成功！' ; ";
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from Firstcustm c ";
	...
	public int Write(JSONObject jsonObject) {
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "\n   begin ";
			qry += "\n    select id into v_id from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "\n    update Firstcustm set amount=" + Amount + ",price=" + Price + ",curr=" + Curr + " where id=v_id and accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
			qry += "\n    insert into Firstcustm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0,lastdate)";
			qry += "\n    values (Firstcustm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + " ," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'',sysdate);";
			qry += "\n  end; ";
		}
		qry += "\n      delete from Firstcustm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from Firstcustm a where accid=" + Accid + " and noteno='" + Noteno + "'";
