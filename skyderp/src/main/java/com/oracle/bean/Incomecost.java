...
//*************************************
//  @author:sunhong  @date:2017-03-26 19:13:02
//*************************************
public class Incomecost implements java.io.Serializable {
	private Float Id;
	private Long Accid;
	...
	public int Delete() {
		...
		strSql.append("\n begin");
//		if (delbj == 1) {
//			strSql.append("\n  update incomecost set statetag=2 ");
//			strSql.append("\n  where accid=" + Accid + " and noteno='" + Noteno + "';");
//		} else {
			// notetype:0销售(退库) 1物料销售（退库）
			// fs:0销售收款勾销售单1销售收款勾退货单2销售退款勾退货单
			strSql.append("\n  select checkman into v_checkman from incomecost where accid=" + Accid + " and noteno='" + Noteno + "';");
			strSql.append("\n  if v_checkman<>'' then");
			strSql.append("\n     v_retcs:='0费用单已审核，不允许删除！';");
			strSql.append("\n     goto exit;");
			strSql.append("\n  end if;");
...
//		}
		...
		if (retcs.substring(0, 1).equals("1")) {
			pFunc.myWriteLog(Accid, "销售费用", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			return 1;
		} else
			return 0;
	...
	public int Append() {
		...
		strSql.append("\n     end if;");
		strSql.append("\n   begin ");
		strSql.append("\n     select 'SF'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from incomecost");
		strSql.append("\n     where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'SF'||To_Char(SYSDATE,'yyyymmdd');");
		strSql.append("\n     v_id:=incomecost_ID.NEXTVAL;");

		strSql.append("\n     insert into incomecost (" + strSql1.toString() + ")");
		strSql.append("\n     values (" + strSql2.toString() + ");");

		strSql.append("\n     commit;");
		strSql.append("\n     v_retbj:=1;  ");
		strSql.append("\n     select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from incomecost where id=v_id;");
		strSql.append("\n     insert into LOGRECORD (id,accid,progname,remark,lastop)");
		strSql.append("\n     values (LOGRECORD_id.nextval," + Accid + ",'销售费用','【增加记录】单据号:'||v_noteno||';金额:" + Curr + "','" + Lastop + "');");
		strSql.append("\n    exception");
		strSql.append("\n    when others then ");
		strSql.append("\n    begin");
		strSql.append("\n      rollback;");
	...
	public DataSet GetList(String strWhere, String fieldlist) {
		...
		strSql.append(" FROM incomecost a");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from incomecost");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n     from incomecost where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
		...
		// qry += "\n end if; ";
		// }
		qry += "\n   update incomecost set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";

		qry += "\n   v_retcs:= '1操作成功！';  ";

		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'销售费用','【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   commit;";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_mess varchar2(200);";
		qry += "\n begin";
		qry += "\n   select checkman,statetag into v_checkman,v_statetag from incomecost where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		if (bj == 1) {// 审核
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update incomecost set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';";
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update incomecost set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';";
		}
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'销售费用',v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
