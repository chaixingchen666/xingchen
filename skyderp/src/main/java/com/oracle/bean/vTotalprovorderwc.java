 ...
 *
 */
public class vTotalprovorderwc {
...
//		Mindate1 += " 00:00:00";
		...
		qry += "\n    declare  ";
		qry += "\n       cursor mycursor is ";
		qry += "\n       select ROWID from v_cxprovorderwc_data where epid=" + Userid;
		qry += "\n       order by rowid; ";
		...
		qry += "\n          delete from v_cxprovorderwc_data where rowid=v_rowid(i); ";
		qry += "\n          commit; ";
		qry += "\n        end loop; ";
		qry += "\n       close mycursor; ";
		qry += "\n    END; ";

		qry += "\n    insert into v_cxprovorderwc_data (id,epid,provid,wareid,colorid,sizeid,amount0,amount1) ";
		qry += "\n    select v_cxprovorderwc_data_id.nextval," + Userid + ",provid,wareid,colorid,sizeid,amount0,amount1 from (  ";
		qry += "\n    select provid,wareid,colorid,sizeid,sum(amount0) as amount0,sum(amount1) as amount1 from (";
		qry += "\n    select a.provid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amount0,0 as amount1 ";
		qry += "\n    from provorderh a  ";
		...
		qry += "\n    union all ";
		// --采购入库
		qry += "\n    select a.provid,b.wareid,b.colorid,b.sizeid,0 as amount0,sum(b.amount) as amount1 ";
		qry += "\n    from wareinh a  ";
		...
		qry += "\n    delete from v_cxprovorderwc_data where epid=" + Userid + " and amount0=0;";
		qry += "\n    select nvl(sum(amount0),0),nvl(sum(amount1),0),nvl(sum(amount0-amount1),0) into :totalamt0,:totalamt1,:totalamt2 from v_cxprovorderwc_data where epid=" + Userid + "; ";
		qry += "\n end;";
		// 申请返回值
	...
	public Table GetTable(QueryParam qp) {
		...
		strSql.append("\n left outer join provide g on a.provid=g.provid");
