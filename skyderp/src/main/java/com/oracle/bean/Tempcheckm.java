...
//*************************************
//  @author:sunhong  @date:2017-03-07 23:00:24
//*************************************
public class Tempcheckm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
		strSql.append("\n   select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   update tempcheckh set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   commit;");
		strSql.append("\n end;");
	...
	// 从xls导入ok
	public int AppendXLS(String wareno, String colorname, String sizename) {
		...
		// decimal Curr =Func.getRound(Amount * Price,2);
		String qry = "declare ";
		qry += "\n    v_totalcurr  number;";
		qry += "\n    v_totalamt  number;";
		qry += "\n    v_wareid  number;";
		...
		qry += "\n begin ";
		qry += "\n   begin ";
		qry += "\n     select wareid,sizegroupno,retailsale into v_wareid,v_sizegroupno,v_retailsale from warecode where accid= " + Accid + " and wareno='" + wareno + "' and statetag=1 and rownum=1;";
		...
		qry += "\n   end; ";
		qry += "\n    begin ";
		qry += "\n      select id into v_id from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update tempcheckm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
		...
		qry += "\n      values (tempcheckm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_retailsale,v_retailsale*" + Amount + ",'');";
		qry += "\n    end; ";
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=tempcheckm_id.nextval; ");
		strSql.append("   insert into tempcheckm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update tempcheckm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from tempcheckm");
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n     select count(*) into v_count from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        insert into tempcheckm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0) ";//
		qry += "\n        values (tempcheckm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update tempcheckm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += "\n update tempcheckh set totalcurr=(select nvl(sum(curr),0) from tempcheckm where tempcheckh.accid=tempcheckm.accid and tempcheckh.noteno=tempcheckm.noteno) ";
		// qry += "\n ,totalamt=(select nvl(sum(amount),0) from tempcheckm where tempcheckh.accid=tempcheckm.accid and tempcheckh.noteno=tempcheckm.noteno ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n     update tempcheckh set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "'; ";
		...
		qry += "\n        values (warebarrecord_id.nextval," + Accid + ",'" + barcode + "',9,'" + Noteno + "',v_amount,sysdate); ";
		qry += "\n     end if;";
		qry += "\n     commit; ";
		qry += "\n     <<exit>> ";
		qry += "\n     select v_retcs into :retcs from dual; ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				qry += "\n   begin ";
				qry += "\n     select id into v_id from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update tempcheckm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into tempcheckm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
				qry += "\n     values (tempcheckm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n    v_retcs  varchar2(100);";
		qry += "\n begin ";
		qry += "\n    update tempcheckm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "  begin ";
			qry += "    select id into v_id from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update tempcheckm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
			qry += "    insert into tempcheckm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
			qry += "    values (tempcheckm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
			qry += " end; ";
		}
		qry += "\n  delete tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";

		qry += "\n  select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "';";

		qry += "\n  update tempcheckh set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
	...
	public int Write(JSONObject jsonObject) {
		...
		qry += "\n     v_id  number;";
		qry += "\n  begin ";
		qry += "\n   delete from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		String sizelist = "^";
		// qry += "\n update tempcheckm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		for (int i = 0; i < jsonArray.size(); i++) {
			JSONObject jsonObject2 = jsonArray.getJSONObject(i);
			...
			// qry += "\n begin ";
			// qry += "\n select id into v_id from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			// qry += "\n update tempcheckm set amount=" + Amount + ",price=" + Price + ",curr=" + Curr + " where id=v_id and accid=" + Accid + " and noteno='" + Noteno + "';";
			// qry += "\n EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
		...
		// qry += "\n delete from tempcheckm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
		// 将商品，颜色未输入的尺码自动填为0，表示数量为0
		qry += "\n     insert into tempcheckm (id,noteno,accid,wareid,colorid,sizeid,amount,price,curr,remark0)";
		qry += "\n     select tempcheckm_id.nextval,'" + Noteno + "'," + Accid + ",a.wareid,b.colorid,c.sizeid,0,a.retailsale,0,'' from warecode a";
		...
		qry += "\n     and not exists (select 1 from tempcheckm e where a.wareid=e.wareid and b.colorid=e.colorid and c.sizeid=e.sizeid";
		qry += "\n     and e.accid=" + Accid + " and e.noteno='" + Noteno + "')";
		qry += "\n     and exists (select 1 from tempcheckm e where a.wareid=e.wareid and b.colorid=e.colorid";
		qry += "\n     and e.accid=" + Accid + " and e.noteno='" + Noteno + "');";
		...
		qry += "\n     commit; ";
		qry += "\n end; ";
		// LogUtils.LogDebugWrite("writetempcheckmsum", qry);
		if (DbHelperSQL.ExecuteSql(qry) < 0) {
			errmess = "操作异常！";
			return 0;
		} else {
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (Wareid != wareid0) {
				SizeParam sp = new SizeParam(Wareid, sizenum, 2);
			...
			qry1 += " from tempcheckm a where accid=" + Accid + " and noteno='" + Noteno + "'";
	...
	public int Load(int priceprec, String wareno) {
		if (Noteno.equals("")) {
			errmess = "wareid或noteno不是一个有效的值！";
			return 0;
		}
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from tempcheckm c ";
