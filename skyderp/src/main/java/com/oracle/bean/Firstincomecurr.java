...
//*************************************
//  @author:sunhong  @date:2017-03-07 21:44:07
//*************************************
public class Firstincomecurr implements java.io.Serializable {
	private Float Id;
	private Long Accid;
...
//		}
		...
		if (DbHelperSQL.ExecuteSql(strSql.toString()) < 0) {
			...
			pFunc.myWriteLog(Accid, "期初应收", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			errmess = "删除成功！";
			return 1;
		}
	}

	public int AppendXLS(String custname, String housename) {
		if (custname.equals("")) {
			errmess = "客户无效!";
			return 0;
		}
		...
		qry += "\n begin ";
		qry += "\n   begin ";
		qry += "\n      select custid into v_custid from customer where custname='" + custname + "' and statetag=1 and noused=0  and accid=" + Accid + " and rownum=1;";
		qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
		qry += "\n      v_retcs:= '0客户:" + custname + " 未找到!' ; ";
		qry += "\n      goto exit;";
		qry += "\n   end ;";
		qry += "\n   begin ";
		...
		qry += "\n   begin ";
		qry += "\n    select 'QS'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from FIRSTincomeCURR";
		qry += "\n    where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'QS'||To_Char(SYSDATE,'yyyymmdd');";
		qry += "\n    v_id:=FIRSTincomeCURR_ID.NEXTVAL;";
		...
		qry += "\n    values (v_id," + Accid + ",v_noteno,sysdate,v_custid ,v_houseid," + Curr + ",'" + Remark + "','" + Handno + "',1,'" + Lastop + "','','" + Lastop + "',sysdate);";
		qry += "\n    commit;";
		qry += "\n    v_retcs:= '1'||v_noteno ;  ";
		qry += "\n  exception";
		qry += "\n  when others then ";
	...
	public int Append() {
		...
		strSql.append("\n     insert into firstincomecurr (" + strSql1.toString() + ")");
		strSql.append("\n     values (" + strSql2.toString() + ");");

		strSql.append("\n     commit;");
		strSql.append("\n     v_retbj:=1;  ");
		strSql.append("\n     select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from firstincomecurr where id=v_id;");
		strSql.append("\n     insert into LOGRECORD (id,accid,progname,remark,lastop)");
		strSql.append("\n     values (LOGRECORD_id.nextval," + Accid + ",'期初应收','【增加记录】单据号:'||v_noteno||';金额:" + Curr + "','" + Lastop + "');");
		strSql.append("\n    exception");
		strSql.append("\n    when others then ");
		strSql.append("\n    begin");
		strSql.append("\n      rollback;");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from Firstincomecurr");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n     from firstincomecurr where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
		qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
		qry += "\n     v_retcs:='0未找到单据!';";
		qry += "\n     goto exit;";
		qry += "\n   end;";
		...
		//			qry += "\n   end if; ";
		//		}
		qry += "\n   update firstincomecurr set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";

		qry += "\n   v_retcs:= '1操作成功！';  ";

		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'期初应收款','【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   commit;";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_statetag number(1);";
		qry += "\n begin";
		qry += "\n   select checkman,statetag into v_checkman,v_statetag from firstincomecurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		if (bj == 1) {// 审核
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update firstincomecurr set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';";
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update firstincomecurr set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';";
		}
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'期初应收',v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
