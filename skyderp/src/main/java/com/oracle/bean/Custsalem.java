...
//*************************************
public class Custsalem implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
		strSql.append("\n   select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from custsalem where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   update custsaleh set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n   commit;");
		strSql.append("\n end;");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=custsalem_id.nextval; ");
		strSql.append("   insert into custsalem (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update custsalem set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public Table GetTable(QueryParam qp, String strWhere, String fieldlist) {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select " + fieldlist);
		strSql.append(" FROM custsalem a");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from custsalem");
	...
	public int LoadFromXLS() {
		...
		qry += "\n   end; ";
		qry += "\n    begin ";
		qry += "\n      select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update custsalem set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
		qry += "\n    EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
		qry += "\n      v_curr:=round(v_price*" + Amount + ",2);";
		qry += "\n      insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
		qry += "\n      values (custsalem_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_price,1,v_price ,v_curr ,'');";
		qry += "\n    end; ";
		qry += "\n    select  nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from custsalem where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    update custsaleh set totalcurr=v_totalcurr,totalamt=v_totalamt where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n    v_retcs:='1操作成功!' ; ";
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n     select a.wareid,a.colorid,a.sizeid,b.retailsale,b.wareno,b.warename,c.colorname,d.sizename ,b.sizegroupno ,d.groupno  ";//
		...
		qry += "\n     select count(*) into v_count from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0) ";//
		qry += "\n        values (custsalem_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,1,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update custsalem set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += "\n update custsaleh set totalcurr=(select nvl(sum(curr),0) from custsalem where custsaleh.accid=custsalem.accid and custsaleh.noteno=custsalem.noteno) ";
		// qry += "\n ,totalamt=(select nvl(sum(amount),0) from custsalem where custsaleh.accid=custsalem.accid and custsaleh.noteno=custsalem.noteno ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from custsalem where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n     update custsaleh set totalcurr=v_totalcurr,totalamt=v_totalamt  ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				Curr = Func.getRound(Amount * Price, 2);
				qry += "\n   begin ";
				qry += "\n     select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update custsalem set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
				qry += "\n     values (custsalem_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + ",1," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from custsalem where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n    v_retcs  varchar2(100);";
		qry += "\n begin ";
		qry += "\n    update custsalem set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "  begin ";
			qry += "    select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update custsalem set amount=amount+" + Amount + ",curr=round((amount+" + Amount + ")*price,2) where id=v_id;";
			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
			qry += "    insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
			qry += "    values (custsalem_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
			qry += " end; ";
		}
		qry += "\n  delete custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";

		qry += "\n  select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from custsalem where accid=" + Accid + " and noteno='" + Noteno + "';";

		qry += "\n  update custsaleh set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
	...
	public int Load(long custid, int priceprec, String wareno) {
		if (Noteno.equals("")) {
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from custsalem c ";
	...
	public int Write(JSONObject jsonObject) {
		...
		qry += "\n begin ";
		qry += "\n      update custsalem set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		//		qry += "\n      delete from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		String sizelist = "^";
		for (int i = 0; i < jsonArray.size(); i++) {
			JSONObject jsonObject2 = jsonArray.getJSONObject(i);
			Sizeid = Long.parseLong(jsonObject2.getString("sizeid"));
			...
			qry += "\n   begin ";
			qry += "\n     select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid //
					+ " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "\n     update custsalem set lastdate=sysdate,amount=" + Amount + ",discount=" + Discount + ",price=" + Price + ",price0=" + Price0 + ",curr=" + Curr + ",remark0='" + Remark0 //
					+ "' where id=v_id and  accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
			qry += "\n     insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0,lastdate)";
			qry += "\n     values (custsalem_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + " ," + Colorid + "," + Sizeid + ","//
					+ Amount + "," + Price0 + "," + Discount + "," + Price + "," + Curr + ",'" + Remark0 + "',sysdate);";
			qry += "\n  end; ";
		}
		qry += "\n      delete from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from custsalem a where accid=" + Accid + " and noteno='" + Noteno + "'";
	...
	public int LoadFromXLS(String gbbar) {
		...
		if (gbbar.length() > 0) {
			qry += "\n   begin ";
			qry += "\n     select wareid,sizegroupno,sale4,retailsale into v_wareid,v_sizegroupno,v_wholesale,v_price0 from warecode where accid= " + Accid + " and gbbar='" + gbbar + "' and statetag=1 and rownum=1;";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
			...
			qry += "\n   begin ";
			qry += "\n     select wareid,sizegroupno,sale4,retailsale into v_wareid,v_sizegroupno,v_wholesale,v_price0 from warecode where accid= " + Accid + " and wareno='" + Wareno + "' and statetag=1 and rownum=1;";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN ";
			qry += "\n     v_retcs:= '0货号" + Wareno + "未找到!' ; ";
			qry += "\n     goto exit; ";
			qry += "\n   end; ";
		...
		qry += "\n    begin ";
		qry += "\n      select id into v_id from custsalem where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update custsalem set amount=amount+" + Amount + ",curr=round((amount+" + Amount + ")*price,2) where id=v_id;";
		qry += "\n    EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
		qry += "\n      insert into custsalem (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
		qry += "\n      values (custsalem_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_price0,v_discount,v_price,round(" + Amount + "*v_price,2),'');";
		qry += "\n    end; ";
	...
	public int ChangeDisc(long custid, int priceprec) {
		...
		if (custid > 0) {// 根据custid查询价格方式及折扣
			...
			qry += "\n     from custsalem a join warecode b on a.wareid=b.wareid";
			qry += "\n     where a.accid=" + Accid + " and a.noteno='" + Noteno + "' and b.tjtag=0; ";
			qry += "\n     v_row cur_data%rowtype;";
			qry += "\n   begin";
			qry += "\n     for v_row in cur_data loop";
			qry += "\n         update custsalem set price0=v_row.price0,discount=v_discount,price=round(v_row.price0*v_discount," + priceprec + ")";
			...
			qry += "   update custsalem set discount=v_discount,price=round(price0*v_discount," + priceprec + "),curr=round(amount*round(price0*v_discount," + priceprec + "),2)";
			qry += "     where accid=" + Accid + " and noteno='" + Noteno + "'; ";
		}
		qry += "   select nvl(sum(curr),0) into v_totalcurr from custsalem where  accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   update custsaleh set totalcurr=v_totalcurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   commit;";
		qry += "end; ";
		if (DbHelperSQL.ExecuteSql(qry) < 0) {
			errmess = "操作失败！";
