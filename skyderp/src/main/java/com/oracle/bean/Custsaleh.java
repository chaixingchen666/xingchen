...
//*************************************
public class Custsaleh implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n begin ");
		strSql.append("\n   select statetag,operant into v_statetag,v_operant from Custsaleh where accid=" + Accid + " and noteno='" + Noteno + "';");
		...
		strSql.append("\n      goto exit; ");
		strSql.append("\n   end if;");
//		strSql.append("\n   delete from Custsalem ");
//		strSql.append("\n   where accid=" + Accid + " and noteno='" + Noteno + "'");
//		strSql.append("\n   and exists (select 1 from Custsaleh a where a.accid=" + Accid + " and a.noteno='" + Noteno + "'");
//		strSql.append("\n   and a.accid=Custsalem.accid and a.noteno=Custsalem.noteno and a.statetag=0);");
//		strSql.append("\n   delete from Custsaleh ");
//		strSql.append("\n   where accid=" + Accid + " and noteno='" + Noteno + "' and statetag=0; ");
		strSql.append("\n   update Custsaleh set statetag=2,lastdate=sysdate");
		strSql.append("\n   where accid=" + Accid + " and noteno='" + Noteno + "' and statetag=0; ");
		strSql.append("\n   v_retcs:='1操作成功！';");
		strSql.append("\n   <<exit>>");
	...
	public int Append() {
		...
		qry += "\n begin ";
		qry += "\n  begin ";
		qry += "\n     select 'KX'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from Custsaleh";
		qry += "\n       where accid=" + Accid + " and LENGTH(noteno)=15 and substr(noteno,1,10)= 'KX'||To_Char(SYSDATE,'yyyymmdd');";
		qry += "\n   v_id:=CustsaleH_ID.NEXTVAL;";
		qry += "\n   insert into Custsaleh (id, accid,noteno,notedate,Custid,statetag,totalamt,totalcurr,remark,handno,operant,checkman,lastdate)";
		qry += "\n   values (v_id," + Accid + ",v_noteno,sysdate," + Custid + ",0,0,0,'','','" + Operant + "','',sysdate);";
		qry += "\n   commit;";
		qry += "\n   v_retbj:=1; select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from Custsaleh where id=v_id;  ";
		qry += "\n    insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n    values (LOGRECORD_id.nextval," + Accid + ",'客户销售','【新增单据】单据号:'||v_noteno,'" + Operant + "');";
		qry += "\n exception";
		qry += "\n  when others then ";
		qry += "\n  begin";
		qry += "\n    rollback;";
	...
	public int Update(int changedatebj) {
		...
		{
			strSql.append("\n  select nvl(sum(curr),0),nvl(sum(amount),0) into v_totalcurr,v_totalamt from Custsalem  where accid=" + Accid + " and noteno='" + Noteno + "';");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from Custsaleh");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n     from Custsaleh where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
		...
		// qry += "\n end if; ";
		// }
		qry += "\n   update Custsaleh set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";

		qry += "\n   v_retcs:= '1操作成功！';  ";

		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'客户销售','【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   commit;";
		// if (changedatebj == 1) // 如果允许更改单据日期， 撤单要更改账套登账日期
		// qry += "\n update accreg set calcdate=trunc(v_notedate-1) where
		// accid=" + Accid + " and calcdate>=trunc(v_notedate);";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_mess varchar2(200);";
		qry += "\n begin";
		qry += "\n   select checkman into v_checkman from Custsaleh where accid=" + Accid + " and noteno='" + Noteno + "';";
		if (bj == 1) {// 审核
			qry += "\n   if length(v_checkman)>0 then ";
			qry += "\n      v_retcs:='0单据已审核！';";
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update Custsaleh set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';";
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update Custsaleh set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';";
		}
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'客户销售',v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
		Map<String, ProdParam> param = new HashMap<String, ProdParam>();
