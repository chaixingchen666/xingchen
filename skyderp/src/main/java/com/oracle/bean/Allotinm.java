...
//*************************************
//  @author:sunhong  @date:2017-03-08 20:55:38
//*************************************
public class Allotinm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.setLength(0);
		// strSql.append(" update allotinm set statetag=2,lastop='" + Lastop + "',lastdate=sysdate");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=allotinm_id.nextval; ");
		strSql.append("   insert into allotinm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update allotinm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from allotinm");
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		qry += "\n     select count(*) into v_count from allotinm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        values (allotinm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from allotinm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update allotinm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += " update allotinh set totalcurr=(select nvl(sum(curr),0) from allotinm where allotinh.accid=allotinm.accid and allotinh.noteno=allotinm.noteno) ";
		// qry += " ,totalamt=(select nvl(sum(amount),0) from allotinm where allotinh.accid=allotinm.accid and allotinh.noteno=allotinm.noteno ) ";
		qry += "\n   select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotinm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		...
		qry += "\n      values (warebarrecord_id.nextval," + Accid + ",'" + barcode + "',3,'" + Noteno + "',v_amount,sysdate); ";
		qry += "\n   end if;";
		qry += "\n   commit; ";
		qry += "\n   <<exit>>";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from allotinm a where accid=" + Accid + " and noteno='" + Noteno + "'";
