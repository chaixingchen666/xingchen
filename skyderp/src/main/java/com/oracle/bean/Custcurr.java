...
//*************************************
public class Custcurr implements java.io.Serializable {
	private Float Id;
	private Long Accid;
...
//			else
				pFunc.myWriteLog(Accid, "客户缴款", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			return 1;
		} else
			return 0;
...
//		}
		...
		strSql.append("\n begin ");
//		if (Fs == 0) {
			strSql.append("\n     select 'KJ'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from custcurr");
			strSql.append("\n     where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'KJ'||To_Char(SYSDATE,'yyyymmdd');");
			strSql.append("\n     v_progname:='客户缴款';");
//		} else {
//			strSql.append("\n     select 'SZ'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from custcurr");
//			strSql.append("\n     where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'SZ'||To_Char(SYSDATE,'yyyymmdd');");
//			strSql.append("\n     v_progname:='销售折让';");
//		}
		strSql.append("\n     v_id:=custcurr_ID.NEXTVAL;");

		strSql.append("\n     insert into custcurr (" + strSql1.toString() + ")");
		strSql.append("\n     values (" + strSql2.toString() + ");");

		strSql.append("\n     v_retbj:=1;  ");
		strSql.append("\n     select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from custcurr where id=v_id;");
		...
		if (Statetag != null && Statetag == 1 ) {//自动登账
			strSql.append("\n  p_paytobook(" + Accid + ",v_noteno,4);"); //结算自动登入账本 fs:0=商场零售，1=店铺零售,2=销售收款，3=采购付款,4=客户缴押金
		}
...
//		}
		...
		if (Statetag != null && Statetag == 1) {//自动登账
			strSql.append("\n  commit;");
			strSql.append("\n  p_paytobook(" + Accid + ",'" + Noteno + "',4);"); //结算自动登入账本 fs:0=商场零售，1=店铺零售,2=销售收款，3=采购付款，4=客户缴押金
		}
		strSql.append("\n   <<exit>>");
		strSql.append("\n   select v_retcs into :retcs from dual;");
		strSql.append("\n end;");
...
//		strSql.append(" left outer join employe e on a.handmanid=e.epid");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from custcurr");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n   begin";
		qry += "\n     select to_char(notedate,'yyyy-mm-dd'),payid,checkman,bd_id ";
		qry += "\n     into v_notedate,v_payid,v_checkman,v_bdid";
		qry += "\n     from custcurr where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
...
//		qry += "\n   select count(*) into v_count from currlink where accid=" + Accid + " and notetype=2 and pnoteno='" + Noteno + "';";
//		qry += "\n   if v_count>0 then ";
...
//		qry += "\n   end if;";
		...
		qry += "\n   update custcurr set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";
		qry += "\n   if v_bdid>0 then";//已登账本，判断账本记录是否已审核，如果已审核，不允许撤单，否则删除账本记录
		...
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'客户缴款','【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_mess varchar2(200);";
		qry += "\n begin";
		qry += "\n   select checkman,payid,statetag into v_checkman,v_payid,v_statetag from custcurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		if (bj == 1) {// 审核
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update custcurr set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';";
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update custcurr set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';";
		}
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'客户缴款',v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
