...
//*************************************
//  @author:sunhong  @date:2017-03-26 19:12:54
//*************************************
public class Paycost implements java.io.Serializable {
	private Float Id;
	private Long Accid;
	...
	public int Delete() {
		...
		strSql.append("delete paycost ");
		strSql.append("  where statetag=0 and noteno='" + Noteno + "' and accid=" + Accid);
		if (DbHelperSQL.ExecuteSql(strSql.toString()) < 0) {
			errmess = "删除失败！";
			return 0;
		} else {
			pFunc.myWriteLog(Accid, "采购费用", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			errmess = "删除成功！";
			return 1;
		}
	}
	...
	public int Append() {
		...
		strSql.append("\n   begin ");
		strSql.append("\n     select 'FF'||To_Char(SYSDATE,'yyyymmdd')||trim(to_char(nvl(to_number(max(substr(noteno,11,5)),'99999'),0)+1,'00000')) into v_noteno from paycost");
		strSql.append("\n     where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'FF'||To_Char(SYSDATE,'yyyymmdd');");
		strSql.append("\n     v_id:=paycost_ID.NEXTVAL;");

		strSql.append("\n     insert into paycost (" + strSql1.toString() + ")");
		strSql.append("\n     values (" + strSql2.toString() + ");");

		strSql.append("\n     commit;");
		strSql.append("\n     v_retbj:=1;  ");
		strSql.append("\n     select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from paycost where id=v_id;");
		strSql.append("\n     insert into LOGRECORD (id,accid,progname,remark,lastop)");
		strSql.append("\n     values (LOGRECORD_id.nextval," + Accid + ",'采购费用','【增加记录】单据号:'||v_noteno||';金额:" + Curr + "','" + Lastop + "');");
		strSql.append("\n    exception");
		strSql.append("\n    when others then ");
		strSql.append("\n    begin");
		strSql.append("\n      rollback;");
...
//		strSql.append(",case To_Char(a.notedate,'yyyy-mm-dd') when To_Char(SYSDATE,'yyyy-mm-dd') then '1' else '0' end as istoday");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from paycost");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n     from paycost where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
...
//			qry += "\n   end if; ";
//		}
		qry += "\n   update paycost set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";

		qry += "\n   v_retcs:= '1操作成功！';  ";

		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'采购费用','【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   commit;";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_mess varchar2(200);";
		qry += "\n begin";
		qry += "\n   select checkman,statetag into v_checkman,v_statetag from paycost where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		if (bj == 1) {// 审核
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update paycost set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';"; 
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update paycost set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';"; 
		}
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",'采购费用',v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
