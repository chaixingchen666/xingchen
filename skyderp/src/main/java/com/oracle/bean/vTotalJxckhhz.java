...
public class vTotalJxckhhz {
	...
	// 汇总
	public int Total(int qxbj, int maxday) {
		// 校验开始查询日期
		...
		if (bj == 1) {
			...
			qry += "\n    select a.custid,a.wareid,a.colorid,a.sizeid,sum(a.amount) as amountqc ";
			// if (jgfs == 1)
			...
			qry += "\n    left outer join custcost c on a.custid=c.custid and a.wareid=c.wareid and c.daystr='" + mindate0 + "'";
			qry += "\n    where a.epid=" + Userid;
			...
			// if (jgfs > 0)
			// {
			qry += "\n    left outer join custcost c on a.custid=c.custid and a.wareid=c.wareid and a.daystr=c.daystr";
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amountcs ";
		// if (jgfs == 1)
		...
		// if (jgfs > 0) {
		// qry += "\n left outer join custcost d on a.custid=d.custid and
		// b.wareid=d.wareid and to_char(a.notedate,'yyyy-mm-dd')=d.daystr";
		// }
		...
		if (qxbj == 1) // 1 启用权限控制
		{
			qry += "    and exists (select 1 from employecust x1 where a.custid=x1.custid and x1.epid=" + Userid + " ) ";
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amountcg ";
		// if (jgfs == 1)
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,-sum(b.amount) as amountct ";
		// if (jgfs == 1)
		...
		// qry += ",0 as currct";
		qry += "    from wareouth a  ";
		qry += "    join wareoutm b on a.accid=b.accid and a.noteno=b.noteno  ";
		qry += "    left outer join warecode c on b.wareid=c.wareid ";
		// + if (jgfs > 0) {
		// qry += " left outer join custcost d on a.custid=d.custid and
		// b.wareid=d.wareid and to_char(a.notedate,'yyyy-mm-dd')=d.daystr";
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amountpy ";
		// if (jgfs == 1)
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amountls ";
		// if (jgfs == 1)
		...
		qry += "\n    select a.custid,b.wareid,b.colorid,b.sizeid,-sum(b.amount) as amountpk ";
		// if (jgfs == 1)
		...
		// qry += "\n where epid=" + Userid + "; ";
		// 计算期末成本金额
		qry += "\n   update v_cxjxchz_temp a set currqm=amountqm*nvl((select costprice from custcost b where a.wareid=b.wareid and a.houseid=b.custid and b.daystr='" + Maxdate + "'),0) ;";
		...
		qry += "\n    select v_cxjxchz_data_id.nextval," + Userid + ",wareid,colorid,sizeid,amountqc,amountcg,amountct,amountcs,amountdr,amountpy,amountls,amountxs,amountxt,amountdc,amountpk,amountqm ";
		...
		// ";
		// qry += "\n
		// ,currqm=currqc+currcg+currct+currcs+currdr+currpy-currls-currxs-currxt-currdc-currpk";

		// qry += "\n where epid=" + Userid + "; ";
		// qry += "\n update v_cxjxchz_data a set currqm=amountqm*(select
		// costprice from custcost b where a.wareid=b.wareid and b.daystr='" +
		// Maxdate + "') where epid=" + Userid + ";";
