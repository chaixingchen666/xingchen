...
//*************************************
public class Paylink implements java.io.Serializable {
	private Float Id;
	private Long Accid;
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=paylink_id.nextval; ");
		strSql.append("   insert into paylink (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update paylink set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Accid != null) {
	...
	public Table GetTable(QueryParam qp) {
		...
		qry += "\n from wareinh a where a.statetag=1 and a.ntid=0 and a.accid=" + Accid + " and a.Provid=" + Provid;
//		qry += "\n and a.notedate<=to_date('" + Notedate + " 23:59:59','yyyy-mm-dd hh24:mi:ss')";
		...
		qry += "\n from wareinh a where a.statetag=1 and a.ntid=1 and a.accid=" + Accid + " and a.Provid=" + Provid;
//		qry += "\n and a.notedate<=to_date('" + Notedate + " 23:59:59','yyyy-mm-dd hh24:mi:ss')";
		qry += "\n and a.notedate>=to_date('" + Accbegindate + " 00:00:00','yyyy-mm-dd hh24:mi:ss')";
		qry += "\n union all";
		// 本单勾对采购应付
		...
		qry += "\n from wareinh a join paylink b on a.accid=b.accid and a.noteno=b.noteno and b.notetype=0";
		// notetype:0采购(退货) 1物料采购(退货) 2销售(退库) 3物料销售（退库）
		qry += "\n where a.statetag=1 and a.accid=" + Accid + " and a.Provid=" + Provid;
		// qry += "\n and a.accid=b.accid and a.noteno=b.noteno and b.notetype=0";
		qry += "\n and b.pnoteno='" + Pnoteno + "' and a.ntid=0";
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from paylink");
	...
	public int doCheck(int opid) {// opid:1=勾单，0=取消勾单
		...
		// paylink-> fs:0采购付款勾采购单1采购付款勾退货单2采购退款勾退货单
		if (opid == 1 && Fs != 0 && Fs != 1) {
			errmess = "fs不是一个有效值！";
			return 0;
		}
		...
		if (opid == 0) {// 取消勾单
			...
			{
				qry += "\n   v_curr:=-v_curr;";
				qry += "\n   update wareinh set gdcurr=gdcurr-v_curr where accid=" + Accid + " and noteno='" + Noteno + "';";
				qry += "\n   update paycurr set gdcurr=gdcurr-v_curr where accid=" + Accid + " and noteno='" + Pnoteno + "';";
				qry += "\n   delete paylink where accid=" + Accid + " and noteno='" + Noteno + "' and pnoteno='" + Pnoteno + "' and notetype=" + Notetype + ";";
				qry += "\n   v_curr:=-v_curr;";
			} else {
				qry += "\n   update wareinh set gdcurr=gdcurr-v_curr where accid=" + Accid + " and noteno='" + Noteno + "';";
				qry += "\n   update paycurr set gdcurr=gdcurr-v_curr where accid=" + Accid + " and noteno='" + Pnoteno + "';";
			...
			{
				// 查询对应的销售单的勾单余额
				qry += "\n   select totalcurr-gdcurr into v_balcurr from wareinh where accid=" + Accid + " and noteno='" + Noteno + "';";
				...
				qry += "\n   insert into paylink (id,accid,noteno,notetype,pnoteno,fs,curr,lastop)";
				qry += "\n   values (paylink_id.nextval," + Accid + ",'" + Noteno + "'," + Notetype + ",'" + Pnoteno + "'," + Fs + ",v_curr,'" + Lastop + "');";
				qry += "\n   v_curr:=-v_balcurr;";
		...
		qry += "\n  select v_curr into :gdcurr from paycurr where accid=" + Accid + " and noteno='" + Pnoteno + "';";
		qry += "\n end;";
		Map<String, ProdParam> param = new HashMap<String, ProdParam>();
