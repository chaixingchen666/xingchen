...
//*************************************
//  @author:sunhong  @date:2017-03-26 17:30:43
//*************************************
public class Paycurr implements java.io.Serializable {
	private Float Id;
	private Long Accid;
	...
	public int Delete() {
		...
		strSql.append("\n  select checkman into v_checkman from paycurr where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("\n  if v_checkman<>'' then ");
		strSql.append("\n     v_retcs:='0付款单已审核，不允许删除！';");
		strSql.append("\n     goto exit;");
		strSql.append("\n  end if;");

		strSql.append("\n  select count(*) into v_count from paylink where accid=" + Accid + " and notetype=0 and pnoteno='" + Noteno + "';");
		strSql.append("\n  if v_count>0 then ");
		strSql.append("\n     v_retcs:='0付款单有勾单记录，不允许删除！';");
		strSql.append("\n     goto exit;");
		strSql.append("\n  end if;");

		strSql.append("\n  update paycurr set statetag=2 ");
		// strSql.Append(" where id=" + id + " and accid=" + accid + " and noteno='" + noteno + "'");
		strSql.append("\n  where accid=" + Accid + " and statetag=0 and noteno='" + Noteno + "';");
		// 删除勾单标志
		// strSql.append("\n delete from paylink where accid=" + Accid + " and pnoteno='" + Noteno + "';");
		strSql.append("\n  v_retcs:='1删除成功！';");
		strSql.append("\n  <<exit>>");
		...
		if (retcs.substring(0, 1).equals("1")) {
			if (Fs == 0)
				pFunc.myWriteLog(Accid, "采购付款", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			else
				pFunc.myWriteLog(Accid, "采购折让", "【删除单据】" + Noteno + ",", Lastop);// 写日志
			return 1;
		} else
			return 0;

		// strSql.append("delete paycurr ");
	...
	public int Append() {
		...
		if (Fs == 0) {
			...
			strSql.append("\n     where accid=" + Accid + "  and LENGTH(noteno)=15 and substr(noteno,1,10)= 'FZ'||To_Char(SYSDATE,'yyyymmdd');");
			strSql.append("\n     v_progname:='采购折让';");
		}
		strSql.append("\n     v_id:=paycurr_ID.NEXTVAL;");

		strSql.append("\n     insert into paycurr (" + strSql1.toString() + ")");
		strSql.append("\n     values (" + strSql2.toString() + ");");

		if (!Func.isNull(Ynoteno)) {
			// notetype:0销售(退库) 1物料销售（退库）
			// fs:0销售收款勾销售单1销售收款勾退货单2销售退款勾退货单
			strSql.append("\n  update paycurr set gdcurr=curr where accid=" + Accid + " and id=v_id;");
			strSql.append("\n  update wareinh set gdcurr=gdcurr+" + Curr + " where accid=" + Accid + " and noteno='" + Ynoteno + "';");
			strSql.append("\n  insert into paylink (id,accid,notetype,noteno,pnoteno,fs,curr,lastop)");
			strSql.append("\n  values (paylink_id.nextval," + Accid + ",0,'" + Ynoteno + "',v_noteno,0," + Curr + ",'" + Lastop + "');");
		}

		strSql.append("\n     v_retbj:=1;  ");
		strSql.append("\n     select '\"msg\":\"操作成功\",\"ID\":\"'||id||'\",\"NOTENO\":\"'||noteno||'\",\"NOTEDATE\":\"'||to_char(notedate,'yyyy-mm-dd hh24:mi:ss')||'\"' into v_retcs from paycurr where id=v_id;");
...
//			strSql.append("\n  p_paytobook(" + Accid + ",v_noteno,3);"); //结算自动登入账本 fs:0=商场零售，1=店铺零售,2=销售收款，3=采购付款
//		}
		strSql.append("\n    exception");
		strSql.append("\n    when others then ");
		strSql.append("\n    begin");
...
//			strSql.append("\n  commit;");
//			strSql.append("\n  p_paytobook(" + Accid + ",'" + Noteno + "',3);"); //结算自动登入账本 fs:0=商场零售，1=店铺零售,2=销售收款，3=采购付款
//		}
...
//		if (Fs == 0)// Fs=1折让
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from paycurr");
	...
	public int Cancel(int changedatebj) {
		...
		qry += "\n     from paycurr where accid=" + Accid + "  and noteno='" + Noteno + "' and statetag=1; ";
		...
		qry += "\n      v_retcs:= '0当前付款是采购单结账时的付款记录，不允许撤单！'; ";
		qry += "\n      goto exit; ";
		qry += "\n   end if; ";
...
//		}
		...
		qry += "\n   select count(*) into v_count from paylink where accid=" + Accid + " and pnoteno='" + Noteno + "';";
		qry += "\n   if v_count>0 then ";
		qry += "\n      v_retcs:='0付款单有勾单记录，不允许撤单！';";
		qry += "\n      goto exit;";
		qry += "\n   end if;";

		qry += "\n   update paycurr set statetag=0 where accid=" + Accid + "  and noteno='" + Noteno + "' ; ";
		qry += "\n   if v_bdid>0 then";//已登账本，判断账本记录是否已审核，如果已审核，不允许撤单，否则删除账本记录
		...
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",decode(v_payid,0,'采购折让','采购付款'),'【撤单】单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   commit;";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
	...
	public int Check(int bj) {
		...
		qry += "\n   v_mess varchar2(200);";
		qry += "\n begin";
		qry += "\n   select checkman,payid,statetag into v_checkman,v_payid,v_statetag from paycurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		if (bj == 1) {// 审核
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update paycurr set checkman='" + Operant + "' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【取消审核】';"; 
			...
			qry += "\n      goto exit;";
			qry += "\n   end if;";
			qry += "\n   update paycurr set checkman='' where accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   v_mess:='【审核】';"; 
		}
		qry += "\n   insert into LOGRECORD (id,accid,progname,remark,lastop)";
		qry += "\n   values (LOGRECORD_id.nextval," + Accid + ",case v_payid when 0 then '采购折让' else '采购付款' end,v_mess||'单据号:" + Noteno + "','" + Operant + "');";
		qry += "\n   v_retcs:='1操作成功！';";
		qry += "\n   <<exit>>";
		qry += "\n   select v_retcs into :retcs from dual;";
		qry += "\n end;";
