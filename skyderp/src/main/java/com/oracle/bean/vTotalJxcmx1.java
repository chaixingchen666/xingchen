...
public class vTotalJxcmx1 {
	...
	public int Total2(int qxbj, int housecostbj, int maxday) {
		...
		qry += "\n    END; ";
		qry += "\n   commit;";
		qry += "\n    insert into v_cxjxcmx_data (id,epid,accid,notedate,noteno,notetype,remark,wlremark,wareid,colorid,sizeid,amount0,price0,curr0,pricerk,currrk,amount1,price1,curr1,pricexs,currxs,amount2,curr2,cost) ";
		qry += "\n    select v_cxjxcmx_data_id.nextval," + Userid + "," + Accid
				+ ",notedate,noteno,notetype,remark,wlremark,wareid,colorid,sizeid,amount0,price0,curr0,pricerk,currrk,amount1,price1,curr1,pricexs,currxs,amount2,curr2,cost from (";
		...
		qry += "\n    ,nvl(sum(a.amount*f.costprice),0) as cost";
		qry += "\n    from waresum a ";
		...
		qry += "\n    ,nvl(b.amount*f.costprice,0) as cost";
		...
		// from v_cxjxcmx_temp); ";
		// --计算余额
		qry += "\n    declare cursor cur_v_cxjxcmx_data is select id,amount0,curr0,amount1,curr1,amount2,curr2,cost from v_cxjxcmx_data where epid=" + Userid + " order by notedate,id;  ";
		...
		// ";
		qry += "\n          v_curr2:=round(v_amount2*v_row.cost/(v_row.amount0+v_row.amount1),2); ";
		//		qry += "\n            v_curr2:=v_amount2*v_row.cost/(v_row.amount0+v_row.amount1); ";
	...
	public int Total1(int qxbj, int cxfs, int housecostbj, int maxday) {
		...
		if (cxfs == 1)
			...
			qry += ",wareid,colorid";
		qry += "\n    ,amount0,curr0,currrk,amount1,curr1,currxs,amount2,curr2,cost) ";
		qry += "\n    select v_cxjxcmx_data_id.nextval," + Userid + "," + Accid + ",notedate,noteno,notetype,remark,wlremark";
		if (cxfs == 1)
			qry += " ,wareid";
		else
			qry += ",wareid,colorid";
		qry += "\n    ,amount0,curr0,currrk,amount1,curr1,currxs,amount2,curr2,cost from (";
		...
		qry += "\n    ,nvl(sum(b.amount*f.costprice),0) as cost";
		qry += "\n    from firsthouseh a  ";
		qry += "\n    join firsthousem b on a.accid=b.accid and a.noteno=b.noteno  ";
		qry += "\n    left outer join warecode c on b.wareid=c.wareid ";
		qry += "\n    left outer join warehouse d on a.houseid=d.houseid ";
		...
		// from v_cxjxcmx_temp); ";
		// --计算余额
		qry += "\n    declare cursor cur_v_cxjxcmx_data is select id,amount0,curr0,currrk,amount1,curr1,currxs,amount2,curr2,cost ";
		qry += "\n    from v_cxjxcmx_data a";
		//		qry += "\n    left outer join warecost f on a.wareid=f.wareid and a.daystr=a.daystr";
		//		if (housecostbj == 1)// 1= 分店铺计算成本
		//			qry += " and a.houseid=f.houseid";
		//		else
		//			qry += " and f.houseid=0";

		qry += "\n  where a.epid=" + Userid + " order by notedate,id;  ";
		// qry += " for update nowait; ";
		qry += "\n      v_row cur_v_cxjxcmx_data%rowtype; ";
		qry += "\n    begin ";
		qry += "\n      v_amount2:=0; v_curr2:=0;";
		...
		qry += "\n          end if;";
		qry += "\n          v_amount2:=v_amount2+v_row.amount0-v_row.amount1; ";
		//		qry += "\n          v_curr2:=v_curr2+v_row.cost; ";
//		qry += "\n          v_curr2:=round(v_amount2*v_row.cost/(v_row.amount0+v_row.amount1),2); ";
	
		qry += "\n          if v_row.amount0+v_row.amount1=0 then v_curr2:=0; else";
		qry += "\n             v_curr2:=round(v_amount2*v_row.cost/(v_row.amount0+v_row.amount1),2); ";
		qry += "\n          end if;";
	...
	public int Total(int qxbj, int cxfs, int housecostbj, int maxday) {
		...
		qry += "\n    insert into v_cxjxcmx_data (id,epid,accid,notedate,noteno,notetype,remark,wlremark,amount0,curr0,currrk,amount1,curr1,currxs,amount2,curr2,cost) ";
		qry += "\n    select v_cxjxcmx_data_id.nextval," + Userid + "," + Accid + ",notedate,noteno,notetype,remark,wlremark,amount0,curr0,currrk,amount1,curr1,currxs,amount2,curr2,cost from (";
		if (bj == 1) {
			qry += "\n    select ' ' as notedate,cast('期初' as nvarchar2(100)) as remark,cast(' ' as nvarchar2(20)) as noteno,'' as notetype,cast(' ' as nvarchar2(20)) as wlremark";
			...
			qry += "\n    ,nvl( sum(round(a.amount*f.costprice,2)) ,0) as cost";
		...
		qry += "\n    select to_char(a.notedate,'yyyy-mm-dd hh24:mi:ss') as notedate,cast('初始' as nvarchar2(100)) as remark,a.noteno ,'QC' as notetype ,d.housename as wlremark";
		qry += "\n    ,sum(b.amount) as amount0,sum(b.curr) as curr0,sum(b.curr) as currrk,0 as amount1,0 as curr1,0 as currxs ,0 as amount2,0 as curr2";
		qry += "\n    ,nvl( sum(round(b.amount*f.costprice,2)) ,0) as cost";
		...
		qry += "\n    select id,amount0,curr0,amount1,curr1,amount2,curr2,cost from v_cxjxcmx_data a";
		qry += "\n    where a.epid=" + Userid + " order by notedate,id;  ";
		// qry += " for update nowait; ";
		qry += "\n      v_row cur_v_cxjxcmx_data%rowtype; ";
		qry += "\n    begin ";
