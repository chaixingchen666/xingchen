...
//*************************************
//  @author:sunhong  @date:2017-03-08 20:57:57
//*************************************
public class Provorderm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=provorderm_id.nextval; ");
		strSql.append("   insert into provorderm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update provorderm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from provorderm");
	...
	public int doBarcodein(String barcode, int pricetype, int priceprec, int qxbj) {
		...
		qry += "\n   begin";
		qry += "\n     select a.wareid,a.colorid,a.sizeid,decode(v_pricetype,0,b.entersale,b.retailsale),b.wareno,b.warename,c.colorname,d.sizename  ";//
		...
		qry += "\n     select count(*) into v_count from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        values (provorderm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price0,v_discount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update provorderm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";
		...
		qry += "\n        values (warebarrecord_id.nextval," + Accid + ",'" + barcode + "',10,'" + Noteno + "',v_amount,sysdate); ";
		qry += "\n     end if;";
		qry += "\n     commit; ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				qry += "\n   begin ";
				qry += "\n     select id into v_id from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update provorderm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into provorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
				qry += "\n     values (provorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price0 + "," + Discount + "," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from provorderm where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int ChangeDisc(long provid, int priceprec) {
		...
		if (provid > 0) {
			...
			qry += "\n     from provorderm a join warecode b on a.wareid=b.wareid";
			qry += "\n     where a.accid=" + Accid + " and a.noteno='" + Noteno + "'; ";
			qry += "\n     v_row cur_data%rowtype;";
			qry += "\n   begin";
			qry += "\n     for v_row in cur_data loop";
			qry += "\n         update provorderm set price0=v_row.price0,discount=v_discount,price=round(v_row.price0*v_discount," + priceprec + ")";
			...
			qry += "\n   v_discount:=" + Discount + ";";
			qry += "\n   update provorderm set discount=v_discount,price=round(price0*v_discount," + priceprec + "),curr=round(amount*round(price0*v_discount," + priceprec + "),2)";
			qry += "\n     where accid=" + Accid + " and noteno='" + Noteno + "'; ";
		}
		qry += "\n   select nvl(sum(curr),0) into v_totalcurr from provorderm where  accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n   update provorderh set totalcurr=v_totalcurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n   commit;";
		qry += "\n end; ";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n begin ";
		qry += "\n    update provorderm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			Curr = Func.getRound(Amount * Price, 2);
			qry += "  begin ";
			qry += "    select id into v_id from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update provorderm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
			qry += "    insert into provorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
			qry += "    values (provorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price0 + "," + Discount + "," + Price + "," + Curr + ",'');";
			qry += " end; ";
		}
		qry += "\n  delete provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";

		qry += "\n  select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from provorderm where accid=" + Accid + " and noteno='" + Noteno + "';";

		qry += "\n  update provorderh set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from provorderm a where accid=" + Accid + " and noteno='" + Noteno + "'";
	...
	public int Load(long provid, long houseid, int priceprec, String wareno) {
		if (Noteno.equals("")) {
			errmess = "wareid或noteno不是一个有效的值！";
			return 0;
		}
...
//			fh = ",";
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from provorderm c ";
	...
	public int Write(JSONObject jsonObject) {
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "  begin ";
			qry += "    select id into v_id from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update provorderm set amount=" + Amount + ",price=" + Price + ",price0=" + Price0 + ",curr=" + Curr + ",discount=" + Discount + " where id=v_id and accid=" + Accid + " and noteno='" + Noteno
					+ "';";

			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
			qry += "    insert into provorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0,lastdate)";
			qry += "    values (provorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + " ," + Colorid + "," + Sizeid + "," + Amount + "," + Price0 + "," + Discount + "," + Price + "," + Curr + ",'',sysdate);";
			qry += " end; ";
		}
		qry += "\n      delete from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
		qry += "\n      select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from provorderm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n      update provorderh set totalcurr=v_totalcurr,totalamt=v_totalamt  ";
		qry += "\n      where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n      commit; ";
	...
	public int LoadFromXLS(String gbbar) {
		...
		qry += "\n   end; ";
		qry += "\n    begin ";
		qry += "\n      select id into v_id from provorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1; ";
		qry += "\n      update provorderm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
		qry += "\n    EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
		qry += "\n      insert into provorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price0,discount,price,curr,remark0)";
		qry += "\n      values (provorderm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid ," + Amount + ",v_price,1,v_price," + Amount + "*v_price,'');";
		qry += "\n    end; ";
