...
//	private Long Provid = (long) -1;
	...
	public int doTotal() {
		...
		qry += "\n    declare  ";
		qry += "\n       cursor mycursor is ";
		qry += "         select ROWID from v_cgthhz_data where epid=" + Userid;
		qry += "\n       order by rowid; ";
		...
		qry += "\n          delete from v_cgthhz_data where rowid=v_rowid(i); ";
		qry += "\n          commit; ";
		qry += "\n        end loop; ";
		qry += "\n       close mycursor; ";
		qry += "\n    END; ";

		qry += "\n    insert into v_cgthhz_data (id,epid,provid,houseid,wareid,amount0,curr0,amount1,curr1) ";
		qry += "\n    select v_cgthhz_data_id.nextval," + Userid + ",provid,houseid,wareid,amount0,curr0,amount1,curr1 from ( ";
		qry += "\n    select provid,houseid,wareid,sum(amount0) as amount0,sum(curr0) as curr0,sum(amount1) as amount1,sum(curr1) as curr1 from (";
		//采购
		qry += "\n    select a.provid,a.houseid,b.wareid,sum(b.amount) as amount0,sum(b.curr) as curr0,0 as amount1,0 as curr1 ";
		qry += "\n    from wareinh a  ";
...
//			qry += "\n    and c.provid=" + Provid;
		...
		qry += "\n   group by a.provid,a.houseid,b.wareid ";
		qry += "\n   union all";
		// 退货
		qry += "\n   select a.provid,a.houseid,b.wareid,0 as amount0,0 as curr0,sum(b.amount) as amount1,sum(b.curr) as curr1 ";
		qry += "\n    from wareinh a  ";
		qry += "\n    join wareinm b on a.accid=b.accid and a.noteno=b.noteno  ";
		qry += "\n    left outer join warecode c on b.wareid=c.wareid ";
...
//			qry += "\n    and c.provid=" + Provid;
		...
		qry += "\n  ) group by  provid,houseid,wareid)  ;";
		qry += "\n    commit;   ";
		qry += "\n    select nvl(sum(amount0),0),nvl(sum(curr0),0),nvl(sum(amount1),0),nvl(sum(curr1),0) ";
		qry += "\n    into :totalamt0,:totalcurr0,:totalamt1,:totalcurr1 from v_cgthhz_data where epid=" + Userid + "; ";
		qry += "\n end; ";
	...
	public Table GetTable(QueryParam qp, int hzfs) {
		...
		if (hzfs == 0) // 按供应商汇总
		{
			qry = " select a.provid,b.provname,sum(a.amount0) as amount0,sum(a.curr0) as curr0,sum(a.amount1) as amount1,sum(a.curr1) as curr1";
			qry += ",case sum(a.curr0) when 0 then 0 else round(sum(a.curr1)/sum(a.curr0)*100,2) end as thrate";
			qry += " FROM v_cgthhz_data a";
			qry += " left outer join provide b on a.provid=b.provid";
			qry += " where a.epid=" + Userid;
			qry += " group by a.provid,b.provname";
