 ...
 *
 */
public class vTotalallotorderwc {
...
//		Mindate1 += " 00:00:00";
		...
		qry += "\n    declare  ";
		qry += "\n       cursor mycursor is ";
		qry += "\n       select ROWID from v_cxallotorderwc_data where epid=" + Userid;
		qry += "\n       order by rowid; ";
		...
		qry += "\n          delete from v_cxallotorderwc_data where rowid=v_rowid(i); ";
		qry += "\n          commit; ";
		qry += "\n        end loop; ";
		qry += "\n       close mycursor; ";
		qry += "\n    END; ";

		qry += "\n    insert into v_cxallotorderwc_data (id,epid,houseid,wareid,colorid,sizeid,amount0,amount1) ";
		qry += "\n    select v_cxallotorderwc_data_id.nextval," + Userid + ",houseid,wareid,colorid,sizeid,amount0,amount1 from (  ";
		qry += "\n    select houseid,wareid,colorid,sizeid,sum(amount0) as amount0,sum(amount1) as amount1 from (";
		qry += "\n    select a.houseid,b.wareid,b.colorid,b.sizeid,sum(b.amount) as amount0,0 as amount1 ";
		qry += "\n    from allotorderh a  ";
		qry += "\n    join allotorderm b on a.accid=b.accid and a.noteno=b.noteno  ";
		...
		qry += "\n    union all ";
		// --调拨出库
		qry += "\n    select a.tohouseid,b.wareid,b.colorid,b.sizeid,0 as amount0,sum(b.amount) as amount1 ";
		qry += "\n    from allotouth a  ";
		...
		if (Brandidlist.length() >0)
			qry += "   and "+Func.getCondition("c.brandid", Brandidlist);
		if (Houseidlist.length() >0)
			qry += "   and "+Func.getCondition("a.tohouseid", Houseidlist);
		if (Seasonname.length() > 0)
			qry += "   and c.seasonname='" + Seasonname + "'";
		if (Prodyear.length() > 0)
			qry += "   and c.prodyear='" + Prodyear + "'";
		...
		qry += "\n    delete from v_cxallotorderwc_data where epid=" + Userid + " and amount0=0;";
		qry += "\n    select nvl(sum(amount0),0),nvl(sum(amount1),0),nvl(sum(amount0-amount1),0) into :totalamt0,:totalamt1,:totalamt2 from v_cxallotorderwc_data where epid=" + Userid + "; ";
		qry += "\n end;";
