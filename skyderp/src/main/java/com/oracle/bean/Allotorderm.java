...
//*************************************
//  @author:sunhong  @date:2017-03-10 20:19:05
//*************************************
public class Allotorderm implements java.io.Serializable {
	private Float Id;
	...
	public int Delete() {
		...
		strSql.append("\n   v_totalamt number; ");
		strSql.append("\n begin ");
		strSql.append("\n   delete from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + "; ");
	...
	public int Append() {
		...
		strSql.append(" begin ");
		strSql.append("   v_id:=Allotorderm_id.nextval; ");
		strSql.append("   insert into Allotorderm (");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update Allotorderm set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Noteno != null) {
	...
	public DataSet GetList(String strWhere, String fieldlist) {
		...
		strSql.append(" FROM Allotorderm ");
		if (!strWhere.equals("")) {
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from Allotorderm");
	...
	public int doBarcodein(String barcode, int qxbj) {
		...
		//店铺调入价格方式
		qry += "\n     begin";
		qry += "\n       select b.pricetype1 into v_pricetype from allotorderh a join warehouse b on a.houseid=b.houseid where a.accid=" + Accid + " and a.noteno='" + Noteno + "';";
		qry += "\n     EXCEPTION WHEN NO_DATA_FOUND THEN";
		qry += "\n       v_pricetype:=0;";
		qry += "\n     end;";

		qry += "\n   begin";
		qry += "\n     select a.wareid,a.colorid,a.sizeid,b.wareno,b.warename,c.colorname,d.sizename  ";//
		...
		qry += "\n     into v_wareid,v_colorid,v_sizeid ,v_wareno,v_warename,v_colorname,v_sizename,v_price  ";
		qry += "\n     from warebarcode a  ";
		...
		qry += "\n     select count(*) into v_count from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' ";//
		...
		qry += "\n        values (allotorderm_id.nextval," + Accid + ",'" + Noteno + "',v_wareid,v_colorid,v_sizeid,v_amount,v_price,v_curr,''); ";//
		qry += "\n     else  ";//
		qry += "\n        select id into v_id from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=v_wareid and colorid=v_colorid and sizeid=v_sizeid and rownum=1 order by id ; ";//
		qry += "\n        update allotorderm set amount=amount+v_amount,curr=(amount+v_amount)*price where id=v_id and noteno='" + Noteno + "' and accid=" + Accid + "; ";
		qry += "\n     end if; ";

		// qry += "\n update allotorderh set totalcurr=(select nvl(sum(curr),0) from allotorderm where allotorderh.accid=allotorderm.accid and allotorderh.noteno=allotorderm.noteno) ";
		// qry += "\n ,totalamt=(select nvl(sum(amount),0) from allotorderm where allotorderh.accid=allotorderm.accid and allotorderh.noteno=allotorderm.noteno ) ";
		qry += "\n     select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		...
		qry += "\n        values (warebarrecord_id.nextval," + Accid + ",'" + barcode + "',12,'" + Noteno + "',v_amount,sysdate); ";
		qry += "\n     end if;";
		qry += "\n     commit; ";
		qry += "\n     <<exit>> ";
	...
	public int AppendHand(JSONObject jsonObject) {
		...
		for (int i = 0; i < colorArray.size(); i++) {
			...
			for (int j = 0; j < sizeArray.size(); j++) {
				...
				qry += "\n   begin ";
				qry += "\n     select id into v_id from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
				qry += "\n     update allotorderm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
				qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
				qry += "\n     insert into allotorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
				qry += "\n     values (allotorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
				qry += "\n   end; ";
			}
		}
		qry += "\n    select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr  from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "';";
	...
	public int doGetColorsumList(QueryParam qp, int sizenum, int priceprec) {
		...
		for (int i = 0; i < count; i++) {
			...
			qry1 += " from allotorderm a where accid=" + Accid + " and noteno='" + Noteno + "'";
	...
	public int Load(long houseid, int priceprec, String wareno) {
		...
		if (Wareid != null && Wareid > 0) {
			...
			// qry += ",case " + pricetype + " when 1 then sale1 when 2 then sale2 when 3 then sale3 when 4 then sale4 when 5 then sale5 else retailsale end as price";
			...
			// qry = "select wareid,wareno,warename,entersale,retailsale,sale1,sale2,sale3,sale4,sale5,units,imagename0 from warecode where accid=" + Accid + " and statetag=1 and wareno='" + wareno + "' and rownum=1";
		} else {
			errmess = "商品id参数无效！";
			return 0;
		}
		// qry = "select wareid,wareno,warename,retailsale,units,imagename0 from warecode where wareid=" + Wareid;
		...
		qry += "\n  and (a.noused=0 or a.noused=1 and exists (select 1 from allotorderm c ";
	...
	public int Appendbat(JSONObject jsonObject, int priceprec) {
		...
		qry += "\n begin ";
		qry += "\n    update allotorderm set amount=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + ";";
		String sizelist = "^";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			qry += "  begin ";
			qry += "    select id into v_id from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "    update allotorderm set amount=amount+" + Amount + ",curr=(amount+" + Amount + ")*price where id=v_id;";
			qry += "  EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，挺插入新记录
			qry += "    insert into allotorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0)";
			qry += "    values (allotorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + "," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'');";
			qry += "  end; ";
		}
		qry += "\n  delete allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0;";

		qry += "\n  select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "';";

		qry += "\n  update allotorderh set totalcurr=v_totalcurr,totalamt=v_totalamt,lastdate=sysdate ";
		if (remark != null)
			qry += ",remark='" + remark + "'";
		if (handno != null)
			qry += ",handno='" + handno + "'";
	...
	public int Write(JSONObject jsonObject) {
		...
		qry += "\n     v_id  number;";
		qry += "\n  begin ";
		qry += "\n      update allotorderm set amount=0,curr=0 where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + "; ";
		String sizelist = "^";
		for (int i = 0; i < jsonArray.size(); i++) {
			JSONObject jsonObject2 = jsonArray.getJSONObject(i);
			Sizeid = Long.parseLong(jsonObject2.getString("sizeid"));
			...
			qry += "\n   begin ";
			qry += "\n     select id into v_id from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and colorid=" + Colorid + " and sizeid=" + Sizeid + " and rownum=1; ";
			qry += "\n     update allotorderm set amount=" + Amount + ",price=" + Price + ",curr=" + Curr + " where id=v_id and accid=" + Accid + " and noteno='" + Noteno + "';";
			qry += "\n   EXCEPTION WHEN NO_DATA_FOUND THEN"; // 未找到，插入新记录
			qry += "\n     insert into allotorderm (id,accid,noteno,wareid,colorid,sizeid,amount,price,curr,remark0,lastdate)";
			qry += "\n     values (allotorderm_id.nextval," + Accid + ",'" + Noteno + "'," + Wareid + " ," + Colorid + "," + Sizeid + "," + Amount + "," + Price + "," + Curr + ",'',sysdate);";
			qry += "\n  end; ";
		}
		qry += "\n      delete from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "' and wareid=" + Wareid + " and amount=0; ";
		qry += "\n      select nvl(sum(amount),0),nvl(sum(curr),0) into v_totalamt,v_totalcurr from allotorderm where accid=" + Accid + " and noteno='" + Noteno + "';  ";
		qry += "\n      update allotorderh set totalcurr=v_totalcurr,totalamt=v_totalamt  ";
		qry += "\n      where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n      commit; ";
