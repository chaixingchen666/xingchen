...
//*************************************
//  @author:sunhong  @date:2017-04-11 12:02:44
//*************************************
public class Wareoutcost implements java.io.Serializable {
	private Float Id;
	private Long Accid;
	...
	public int Delete() {
		...
		strSql.append("   select ntid into v_ntid from wareouth where accid=" + Accid + " and noteno='" + Noteno + "';");
		strSql.append("   if v_ntid=2 then ");
		strSql.append("      v_totalcurr:=v_totalcurr-v_totalcost;");
		strSql.append("   else");
		strSql.append("      v_totalcurr:=v_totalcurr+v_totalcost;");
		strSql.append("   end if;");

		strSql.append("\n  update wareouth set totalcurr=v_totalcurr,totalcost=v_totalcost where accid=" + Accid + " and noteno='" + Noteno + "';");
	...
	public int Save(JSONObject jsonObject) {
		...
		qry += "\n   v_totalcurr number; ";
		qry += "\n begin ";
		qry += "\n   delete from wareoutcost where accid=" + Accid + " and noteno='" + Noteno + "';";
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			if (Curr != 0) {
				...
				qry += "\n   values (wareoutcost_id.nextval," + Accid + ",'" + Noteno + "'," + Cgid + "," + Curr + ");";
				totalcost += Curr;
			}
		}
		qry += "\n   select nvl(sum(curr),0) into v_totalcost from  wareoutcost where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n   select nvl(sum(curr),0) into v_totalcurr from  wareoutm where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n   update wareouth set totalcurr=v_totalcurr+decode(ntid,2,-v_totalcost,v_totalcost),totalcost=v_totalcost where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "\n end;";
	...
	public int Append() {
		...
		qry += " begin ";
		qry += "   v_id:=wareoutcost_ID.NEXTVAL;";
		qry += "   insert into wareoutcost (id, accid,noteno,cgid,curr,lastdate)";
		qry += "   values (v_id," + Accid + ",'" + Noteno + "'," + Cgid + "," + Curr + ",sysdate);";
		qry += "   select nvl(sum(curr),0) into v_totalcost from wareoutcost where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   select nvl(sum(curr),0) into v_totalcurr from wareoutm where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   select ntid into v_ntid from wareouth where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   if v_ntid=2 then ";
		qry += "      v_totalcurr:=v_totalcurr-v_totalcost;";
		qry += "   else";
		qry += "      v_totalcurr:=v_totalcurr+v_totalcost;";
		qry += "   end if;";
		qry += "   update wareouth set totalcost=v_totalcost,totalcurr=v_totalcurr where accid=" + Accid + " and noteno='" + Noteno + "';";
		qry += "   commit;";
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update wareoutcost set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Accid != null) {
	...
	public DataSet GetList(String strWhere, String fieldlist) {
		...
		strSql.append(" FROM wareoutcost ");
	...
	public Table GetTable(QueryParam qp, Integer all) {
		...
		if (all == 1) {
			...
			strSql.append("\n and not exists (select 1 from wareoutcost a where a.noteno='" + Noteno //
	...
	public Table GetTable(QueryParam qp, String strWhere, String fieldlist) {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select " + fieldlist);
		strSql.append(" FROM wareoutcost a ");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from wareoutcost");
