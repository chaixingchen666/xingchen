...
//*************************************
public class Employehouse implements java.io.Serializable {
	private Float Id;
	private Long Epid;
	private Long Accid;
	...
	public int Delete() {
		...
		strSql.setLength(0);
		// strSql.append(" update employehouse set statetag=2,lastop='" + Lastop + "',lastdate=sysdate");
	...
	public int Append(int value) {
		...
		if (value == 0) {
			...
			strSql.append("\n    if v_count>0 then ");
			strSql.append("\n       v_retcs:='0该店铺已授权允许入库，不允许取消查看授权！';");
			strSql.append("\n       goto exit;");
			strSql.append("\n    end if;");
			strSql.append("\n    select count(*) into v_count from employehouseout where epid=" + Epid + " and houseid=" + Houseid + ";");
			strSql.append("\n    if v_count>0 then ");
			strSql.append("\n       v_retcs:='0该店铺已授权允许出库，不允许取消查看授权！';");
			strSql.append("\n       goto exit;");
			strSql.append("\n    end if;");

			strSql.append("\n    delete from employehouse a where epid=" + Epid + " and houseid=" + Houseid);
	...
	public int AppendAll(int value) {
		...
		strSql.append(" begin");
		if (value == 0) {
			strSql.append("\n   delete from employehouse a where epid=" + Epid);
			strSql.append("\n   and not exists (select 1 from employe b where a.epid=b.epid and a.houseid=b.houseid)");
			strSql.append("\n   and not exists (select 1 from employehousein b1 where a.epid=b1.epid and a.houseid=b1.houseid)");
			strSql.append("\n   and not exists (select 1 from employehouseout b2 where a.epid=b2.epid and a.houseid=b2.houseid);");	
		} else {
			strSql.append("\n  insert into employehouse (id,epid,houseid)");
	...
	public int Update() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("begin ");
		strSql.append("   update employehouse set ");
		if (Id != null) {
			strSql.append("id='" + Id + "',");
		}
		if (Epid != null) {
	...
	public DataSet GetList(String strWhere, String fieldlist) {
		...
		strSql.append(" FROM employehouse ");
		if (!strWhere.equals("")) {
	...
	public Table GetTable(QueryParam qp) {
		StringBuilder strSql = new StringBuilder();
		strSql.append(" select a.houseid,b.housename,'1' as selbj from employehouse a");
		...
		strSql.append(" and not exists (select 1 from employehouse b where a.houseid=b.houseid and b.epid=" + Epid + ")");
	...
	public Table GetTable1(QueryParam qp) {
		StringBuilder strSql = new StringBuilder();
		strSql.append(" select a.houseid,b.housename from employehouse a");
	...
	public boolean Exists() {
		StringBuilder strSql = new StringBuilder();
		strSql.append("select count(*) as num  from employehouse");
