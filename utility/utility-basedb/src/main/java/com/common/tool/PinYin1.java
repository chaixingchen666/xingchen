...
public class PinYin1 {
	...
	private static String[] strChineseCharList = {
			...
			"H櫘繪翽譓闠鐬靧譿顪昏昬荤婚涽阍惽棔睧睯閽忶浑馄渾魂繉鼲诨俒倱圂掍混焝溷慁觨諢吙耠锪劐鍃豁攉騞佸活秮火伙邩钬鈥夥沎或货咟俰捇眓获剨祸貨惑旤湱禍嗀奯濩獲霍檴謋穫镬嚯瀖", "H耯藿蠖嚿曤臛癨矐鑊夻行砉圜", // 5_1_a_s_p_x.c_o_m
			...
			"L纶侖轮倫陯圇婨崘崙惀淪菕棆腀碖綸蜦踚輪磮錀鯩稐耣论埨溣論捋頱囉罗啰猡脶萝逻椤腡锣箩骡镙螺羅覶鏍儸覼騾玀蘿邏欏鸁籮鑼饠驘剆倮砢蓏裸躶瘰蠃臝攞曪癳泺峈洛络荦骆珞笿絡落", "L摞漯犖雒駱鮥鵅纙鱳", // 5_1_a_s_p_x.c_o_m
	...
	public static String getPinYinHeadChar(String strText) {
		...
		// char a=charArray[0];
		for (Character vChar : charArray) {
			// for (Char vChar in strText)
			String fh = "~!@#$%^&*()-+|{}[]<>?";
			// 若是字母则直接输出
			if ((vChar >= 'a' && vChar <= 'z') || (vChar >= 'A' && vChar <= 'Z') || (vChar >= '0' && vChar <= '9')
				...
				for (String strList : strChineseCharList) {
					if (strList.indexOf(vChar) > 0) {
						myStr.append(strList.toCharArray()[0]);
						break;
					}
					// strList.toCharArray()g
				}
			}
